...
class BasicConv3d(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, stride, padding=0):
        ...
        # verify default value in sonnet
...
class I3D(nn.Module):
    def __init__(self, num_classes=400, dropout_drop_prob=0.5, input_channel=3, spatial_squeeze=True):
        ...
        self.features = nn.Sequential(
            ...
            BasicConv3d(64, 192, kernel_size=3, stride=1, padding=1),    # (192, 32, 28, 28)
            ...
            Mixed_5b(),  # (832, 8, 7, 7)
            Mixed_5c(),  # (1024, 8, 7, 7)
            # nn.AvgPool3d(kernel_size=(2, 7, 7), stride=1),      # (1024, 8, 1, 1) (x) / (1024, 7, 1, 1) (o)
            nn.AvgPool3d(kernel_size=(1, 7, 7), stride=1),      # (1024, 8, 1, 1)
            # change layers for pre-training
            nn.Dropout3d(dropout_drop_prob),
            nn.Conv3d(1024, num_classes, kernel_size=1, stride=1, bias=True),  # (200, 8, 1, 1)
        )
    ...
    def forward(self, x):
        ...
        #     logits = logits.squeeze(3)      # (B, 1024, 8, 1, 1) -> (B, 1024, 8, 1)
        #     logits = logits.squeeze(3)      # (B, 1024, 8, 1) -> (B, 1024, 8)
        #
        # averaged_logits = torch.mean(logits, 2)
        # predictions = nn.Softmax(averaged_logits)
        #
        # self.iter += 1

        for n, model in enumerate(self.features):
            x = model(x)
            if n == 16:     # avgpool layer index is 16.
                # feature_maps = x
                # return feqture maps, averaged_logits, predictions
                # return feature_maps, logits, predictions
                # return feature_maps
                return x
...
if __name__ == '__main__':
    ...
    map, _, _ = rgb_i3d(torch.zeros((1, 3, 64, 224, 224)))
    print(map.size())
