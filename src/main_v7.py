...
loss_dir = os.path.join(logs_dir, "loss", mode)
score_dir = os.path.join(logs_dir, "score", mode)
if not os.path.exists(loss_dir):
    os.makedirs(loss_dir)
if not os.path.exists(score_dir):
    os.makedirs(score_dir)

loss_file = os.path.join(loss_dir, "{}_loss_{}_v{}".format(mode, now.strftime('%Y-%m-%d'), FLAGS.version))
score_file = os.path.join(score_dir, "{}_score_{}_v{}".format(mode, now.strftime('%Y-%m-%d'), FLAGS.version))
...
if mode == "train":
    ...
    ckpt_date = sorted(glob.glob(logs_dir + "/ckpt/*"))[-1]
...
def train():
    ...
    load_dir = None
    # load_dir = "/media/pjh/2e8b4b6c-7754-4bf3-b610-7e52704614af/dense-captioning/logs/ckpt/ckpt_2019-11-15_v4_4/model-80000.pt"
    if load_dir:
    ...
    for epoch in range(epochs):
        ...
        for n in range(iterations):
            ...
            print("\t[1st event: {}]".format(data_batcher.selected_events[0]))
            ...
            if (global_step % DISP_INTERVAL == 0) | (global_step == 0):
                scores = utils.calc_scores(cur_events=data_batcher.selected_events, output=output_argmax,
                                           trg_real=trg_real,
                                           word2ix=word2ix)
                ...
                with open(loss_file, "a") as f:
                    if global_step == 0:
                        ...
                        train_loss = 0
                    f.write("\t[1st event: {}]\n".format(data_batcher.selected_events[0]))
