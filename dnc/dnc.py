...
class DNC:
    ...
    def __init__(self, controller_class, input_size, output_size, keep_prob, initial_memory_state=None,
        ...
        self.controller = controller_class(self.input_size, self.output_size, self.read_heads, self.word_size, keep_prob, self.batch_size)
    ...
    def build_graph(self, input_data, initial_memory_state):
        """
        ...
        # input data : time_major
        self.unpacked_input_data = utility.unpack_into_tensorarray(input_data, 1, sequence_length)

        outputs = tf.TensorArray(tf.float32, sequence_length)
        free_gates = tf.TensorArray(tf.float32, sequence_length)
        allocation_gates = tf.TensorArray(tf.float32, sequence_length)
        write_gates = tf.TensorArray(tf.float32, sequence_length)
        read_weightings = tf.TensorArray(tf.float32, sequence_length)
        write_weightings = tf.TensorArray(tf.float32, sequence_length)
        usage_vectors = tf.TensorArray(tf.float32, sequence_length)

        controller_state = self.controller.get_state() if self.controller.has_recurrent_nn else (tf.zeros(1), tf.zeros(1))

        memory_state = self.memory.init_memory(initial_memory_state)
        ...
        self.packed_memory_view = {
            'new_memory_state' : final_results[1],
            ...
            'final_nn_state': final_results[9]
        }
    ...
    def save(self, session, ckpts_dir, name):
        ...
        ckpts_dir: string
            the path to the ckpt directories
        name: string
