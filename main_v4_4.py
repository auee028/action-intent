...
import utils_v4_4 as utils
from data_loader_v4_4 import FrameDataset
from model_v4_4 import I3D_Transformer
...
loss_file = os.path.join(logs_dir, "loss", "{}_loss_{}_v{}".format(FLAGS.mode, now.strftime('%Y-%m-%d'), FLAGS.version))
score_file = os.path.join(logs_dir, "score", "{}_score_{}_v{}".format(FLAGS.mode, now.strftime('%Y-%m-%d'), FLAGS.version))
